...
function RequestContextModal({ topic, token, onClose }) {
...
function AxisMessage({ msg, onFeedback, onPreview, onRequestContext }) {
  const { content, sources, feedback } = msg
  const canVote = Boolean(msg.id && onFeedback)
  const isStreaming = msg.streaming
  const noSources = !isStreaming && content && msg.noContext
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        {noSources && (
          <div style={{
            ...
            <span style={{ fontSize: 13, color: 'var(--text2)', flex: 1 }}>Need more context from a colleague?</span>
            <button
              onClick={() => onRequestContext && onRequestContext(msg.question || content?.slice(0, 120) || 'General question')}
              style={{
              ...
              onMouseLeave={e => { e.currentTarget.style.background = 'var(--accent-dim)'; e.currentTarget.style.borderColor = 'rgba(99,102,241,0.35)' }}
            >
              Request context &rarr;
            </button>
          </div>
        )}
        {canVote && !isStreaming && (
...
export default function ChatArea({ messages, loading, onSuggest, onFeedback, onRequestContext, token }) {
  ...
  const handleRequestContext = useCallback((topic) => {
    if (onRequestContext) {
      onRequestContext(topic)
    } else {
